    ...
    (() => {
        ...
        function buttonUp() {
            ...
            window.addEventListener("scroll", (() => {
                ...
                const btnUp = document.querySelector(".btn-up");
                scrollY > 400 ? show(btnUp) : hide(btnUp);
            }));
            btnUp.onclick = () => {
                window.scrollTo({
                    top: 0,
        ...
        function formQuantity() {
            document.addEventListener("click", (function(e) {
                ...
                if (targetElement.closest(".quantity__button")) {
                    ...
                    if (isNaN(value)) value = 1; else if (targetElement.classList.contains("quantity__button_plus")) value++; else {
                        --value;
                        if (value < 1) value = 1;
                    }
        ...
        if (ranges.length) ranges.forEach((range => {
            ...
            let from = range.querySelector("[data-from]");
            let to = range.querySelector("[data-to]");
            let inputs = [ from, to ];
            nouislider.create(slider, {
                start: [ Number(from.value), Number(to.value) ],
                connect: true,
                range: {
                    min: Number(from.dataset.from),
                    max: Number(to.dataset.to)
                },
                tooltips: [ true, true ]
            });
                ...
                input.addEventListener("keydown", (function(e) {
                    ...
                    switch (e.which) {
                      ...
                      case 38:
                        ...
                        if (null !== position) slider.noUiSlider.setHandle(handle, value + position);
                        break;
        ...
        function initSliders() {
            ...
            if (document.querySelector(".catalog__slider")) new core(".catalog__slider", {
                ...
                loop: true,
                pagination: {
                    el: ".catalog-controll__dots",
                    clickable: true
                },
                navigation: {
                    prevEl: ".catalog-controll__prev",
                    nextEl: ".catalog-controll__next"
                },
                on: {}
            });
            if (document.querySelector(".product-sliders")) {
                const product__thumbs = new core(".thumbs-slider", {
                    modules: [ Navigation, Pagination ],
                    observer: true,
                    observeParents: true,
                    slidesPerView: 4,
        ...
        da.init();
        const spoilerArrow = document.querySelector(".phones-header__arrow");
        spoilerArrow.addEventListener("click", (() => {
            spoilerArrow.classList.toggle("arrow-rotate");
        }));
        const menu = document.querySelector(".menu__body");
        const burger = document.querySelector(".icon-menu");
        burger.addEventListener("click", (() => {
            menu.classList.toggle("menu-open");
            document.querySelectorAll("._catalogMenu-open").forEach((element => {
                element.classList.remove("_catalogMenu-open");
            }));
            if (document.querySelector("._submenu-active")) {
                const temp = document.querySelector("._parent-active");
                temp.classList.remove("_parent-active");
                document.querySelector("._submenu-active").classList.remove("_submenu-active");
            }
        }));
        document.addEventListener("click", documentActions);
        function documentActions(e) {
            const targetElement = e.target;
            if (targetElement.closest(".top-header-menu__item_catalog-link")) {
                e.preventDefault();
                const menuCatalog = document.querySelector(".menu-catalog__list");
                menuCatalog.classList.add("_catalogMenu-open");
            }
            if (targetElement.closest(".back")) {
                e.preventDefault();
                if (targetElement.closest(".submenu-catalog__block")) {
                    const temp = document.querySelector("._parent-active");
                    temp.classList.remove("_parent-active");
                    targetElement.closest(".closest-for-back").classList.remove("_submenu-active");
                } else targetElement.closest(".closest-for-back").classList.remove("_catalogMenu-open");
            }
        }
        document.addEventListener("click", documentActions2);
        function documentActions2(e) {
            const targetElement = e.target;
            if (targetElement.closest("[data-parent]")) {
                e.preventDefault();
                const submenuId = targetElement.dataset.parent;
                const submenu = document.querySelector(`[data-submenu="${submenuId}"]`);
                let do_addClass = true;
                const temp = document.querySelector("._parent-active");
                if (temp) {
                    if (targetElement.classList.contains("_parent-active")) do_addClass = false;
                    temp.classList.remove("_parent-active");
                    document.querySelector("._submenu-active").classList.remove("_submenu-active");
                }
                if (do_addClass) {
                    targetElement.classList.add("_parent-active");
                    submenu.classList.add("_submenu-active");
                }
            }
        }
        const submenuBlocks = document.querySelectorAll(".submenu-catalog__block");
        submenuBlocks.forEach((block => {
            const id = block.querySelectorAll(".submenu-catalog__category").length;
            block.classList.add("submenu-catalog__block_" + id);
        }));
        const productsMore = document.querySelectorAll(".products-slider__more");
        productsMore.forEach((element => {
            if ("Перейти в каталог" == element.innerText) element.addEventListener("click", (e => {
                e.preventDefault();
                window.location.href = "catalog.html";
            }));
        }));
        const catalogFiterTitle = document.querySelector(".catalog-filter__title");
        if (catalogFiterTitle) catalogFiterTitle.addEventListener("click", (() => {
            if (window.innerWidth < 992) document.querySelector(".catalog-filter__items").classList.toggle("_active");
        }));
        window["FLS"] = true;
        isWebp();
        menuInit();
